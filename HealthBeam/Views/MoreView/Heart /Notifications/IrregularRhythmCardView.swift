...
struct IrregularRhythmCardView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                Text("Irregular Rhythm").font(.headline.bold())
                Spacer()
            }
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                Divider()
                Text("Notifications regarding irregular heart rhythms suggestive of AFib.")
                ...
                VStack(alignment: .leading, spacing: 8) {
                    if let demoEntry = entry {
                        ...
                        HStack {
                            ...
                            Spacer()
                            HStack(spacing: 4) {
                                Text(demoEntry.value) // "Atrial Fibrillation"
        ...
        .sheet(isPresented: $showHistory) { HeartEventHistoryView(title: "Irregular Rhythm History", events: events, color: themeColor, icon: "exclamationmark.triangle.fill") }
    }
    ...
    private func eventRow(_ event: HeartRateEvent) -> some View {
        HStack {
            ...
            Spacer()
            HStack(spacing: 4) { Text("Alert"); Image(systemName: "exclamationmark.triangle.fill") }
            .font(.caption.bold()).foregroundStyle(themeColor)
            .padding(.horizontal, 8).padding(.vertical, 4).background(themeColor.opacity(0.1), in: Capsule())
        }.padding(.vertical, 4)
    }
    ...
    private func load() {
        Task {
            isLoading = true
            let result = await healthKitManager.fetchIrregularRhythmEvents(range: range)
