...
struct SleepApneaCardView: View {
    ...
    let themeColor = Color.indigo

    var body: some View {
        VStack(spacing: 0) {
            HStack {
                Image(systemName: "moon.zzz.fill").font(.title2)
                Text("Sleep Apnea").font(.headline.bold())
                Spacer()
            }
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                HStack(alignment: .firstTextBaseline) {
                    if let _ = entry {
                        ...
                        Text("notification").font(.headline).foregroundStyle(.secondary)
                    } else {
                        if isLoading { ProgressView() } else {
                            ...
                            Text("notifications").font(.headline).foregroundStyle(.secondary)
                        }
                    }
                    Spacer()
                }
                ...
                Divider()
                Text("Possible sleep apnea detected based on breathing disturbance data.")
        ...
        .sheet(isPresented: $showHistory) { HeartEventHistoryView(title: "Sleep Apnea History", events: events, color: themeColor, icon: "moon.zzz.fill") }
    }
    ...
    private func eventRow(_ event: HeartRateEvent) -> some View {
        HStack {
            ...
            Spacer()
            HStack(spacing: 4) { Text("Possible Apnea"); Image(systemName: "exclamationmark.bubble.fill") }
            .font(.caption.bold()).foregroundStyle(themeColor)
            .padding(.horizontal, 8).padding(.vertical, 4).background(themeColor.opacity(0.1), in: Capsule())
        }.padding(.vertical, 4)
    }
    ...
    private func load() {
        if #available(iOS 18.0, *) {
            Task {
                isLoading = true
                let result = await healthKitManager.fetchSleepApneaEvents(range: range)
